...
checks = {
    ...
    'bend time compensates player and mount systems': all(name in time_control for name in ('MaxSpeedMultiplier', 'CombatMaxSpeedMultiplier', 'SwingSpeedMultiplier', 'ReloadSpeed', 'BipedalRangedReadySpeedMultiplier', 'BipedalRangedReloadSpeedMultiplier', 'MountSpeed', 'MountManeuver', 'MountDashAccelerationMultiplier')) and 'channel < NativeActionChannelCount' in time_control,
    'bend time never writes unverified native action channels': 'NativeActionChannelCount = 2' in time_control and time_control.count('channel < NativeActionChannelCount') == 2 and 'channel < 4' not in time_control,
