...
7. Bend Time combines native mission slowdown with player-only driven-property, action-channel, speed-limit, and main-agent-controller delta compensation. Final rider, mount, attack, and camera feel still requires Bannerlord 1.3.15 runtime testing.
...
9. Blink and Voidstep indicators use the camera ray across the complete circular range. Camera yaw selects direction and camera pitch selects distance; static mission geometry and safe-standing validation remain authoritative.
10. Domino damage propagation uses the finalized collision damage and `Blow.OwnerId` when Bannerlord's supplied affector is missing or not the controlled player. Death propagation remains a separate optional MCM setting.
11. Source and invariant checks can prove ordering and ownership properties, while final skeletal pose, foot placement and action compatibility still require Bannerlord 1.3.15 runtime testing.
