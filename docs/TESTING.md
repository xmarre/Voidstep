...
Progression source invariants additionally enforce:

- versioned, hero-keyed campaign persistence;
- no periodic campaign, hourly or daily tick;
- no mission-agent storage in campaign state;
- one immutable, volatile cached runtime profile;
- allocation-free thread-local mission scoping;
- explicit gates for all six abilities;
- disabled-progression passthrough to original behaviour;
- bounded weak-lifetime XP throttling;
- a 19-skill catalogue containing every branch;
- a fully reachable final capstone inside the 99-point budget;
- the deferred Character-screen transition;
- explicit mastery-screen teardown;
- complete Gauntlet bindings and package requirements.
...
## Required mastery campaign matrix

A progression release must record pass/fail and logs for:

- loading an existing pre-progression save creates empty mastery state without enabling progression or changing current ability behaviour;
- starting a new campaign creates independent mastery state for the main hero;
- progression enabled/disabled state persists through save, quit and reload;
- mastery XP, rank and all invested skill levels persist through save, quit and reload;
- respec refunds every invested point, retains XP/rank and persists after reload;
- the available-point count always equals mastery rank minus invested levels and never becomes negative;
- reaching rank 99 permits the documented complete prerequisite route and ten ranks of Avatar of the Void;
- melee-skill requirements use the highest current One Handed, Two Handed or Polearm value;
- changing the main hero during a multi-character campaign resolves the correct hero-keyed mastery record;
- disabling progression restores unrestricted original MCM behaviour immediately and after reload;
- re-enabling progression restores the saved skill profile without duplicating XP or points;
- `voidstep.add_mastery_xp <amount>` adds the scaled XP once and cannot exceed the rank-99 threshold;
- a rank increase displays one notification and exposes the correct new point count.

## Required mastery UI matrix

- the native Character screen shows one **Voidstep Mastery** button without covering the Guided Arrow progression button;
- pressing the button closes the native Character state, returns to the campaign map, settles it and opens the mastery screen without a crash or black screen;
- `Ctrl+Shift+V` opens the same mastery screen only from the campaign map;
- the shortcut does nothing in missions, menus, loading screens and non-map campaign screens;
- `voidstep.open_mastery` opens only from a valid campaign map;
- all six branch columns render at 16:9, 16:10, ultrawide and supported UI-scale variants;
- selecting every node refreshes name, level, requirements, current effect, next effect and status;
- investing one point changes only the selected skill and refreshes the point counter immediately;
- unmet rank, melee, prerequisite and point requirements prevent investment with the correct reason;
- maximum-level nodes reject further investment;
- enable/disable and respec actions update the screen without reopening it;
- Back releases focus, input restrictions, movie and view model and returns to the campaign map;
- campaign exit while the mastery screen is open closes the screen before progression state detaches;
- module unload or return to the main menu leaves no stale overlay, cursor ownership or view-model callback;
- rapidly clicking the Character button or repeatedly pressing the shortcut never pushes duplicate mastery screens.

## Required mastery runtime matrix

With progression enabled:

- Void Affinity rank 1 unlocks Cleave and rank 0 rejects it before resource payment;
- Rift Step rank 1 unlocks Blink and rank 0 rejects it before entering frozen targeting;
- Gale Force rank 1 unlocks Windblast;
- Bend the Hour rank 1 unlocks Bend Time;
- Fateful Link rank 1 unlocks Domino;
- Umbral Sight rank 1 unlocks Dark Vision;
- Dark Vision can always be toggled off after its unlock is removed by respec;
- locked direct selectors and TOR/standalone wheel selections produce one visible rejection and do not consume energy or cooldown;
- raw MCM values remain unchanged in the menu while effective mission values follow the cached mastery profile;
- maximum energy and regeneration never exceed the current progression cap before Avatar of the Void rank 10;
- Avatar of the Void rank 10 releases only the progression cap and preserves the configured MCM value;
- energy costs and cooldowns use the documented skill multipliers and never fall below the 35% floor;
- Momentum Weave rank 1 permits the configured Blink momentum option; rank 0 suppresses it;
- Chronomancer rank 10 permits configured complete suspension; rank 9 suppresses it;
- Unbound Power rank 5 permits configured cooldown-only mode; rank 4 suppresses it;
- Unbound Power rank 10 permits configured unlimited energy; rank 9 suppresses it;
- successful Blink, Windblast, Bend Time, Domino and Dark Vision casts award XP once inside their throttle interval;
- failed targeting, failed payment, cooldown rejection and locked abilities award no XP;
- Cleave awards XP only after completion with at least one successfully registered hit and scales within the documented cap;
- repeated mission starts and ends do not retain XP-throttle state from the previous mission;
- a sustained battle shows no new per-frame allocation or campaign-map performance regression attributable to progression.
...
- progression disabled, newly enabled and heavily invested profiles;
